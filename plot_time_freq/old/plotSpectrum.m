function ha=plotSpectrum(varargin)

%% 

% plotSpectrum(f,S1,S2,S3)
% plotSpectrum(f1,S1,f2,S2,f3,S3)

% Inputs:
% f: frequency axis
% S: (n1*n2*n_freq) 3d PSD matrix
...
addParameter(p,'xlim',2,@isnumeric)
...
if length(xlim_freq)==1;
    xlim_freq=[0 xlim_freq];
end
...
elseif isempty(xLabels);
    xLabels=repcell('Frequency',1,nSignals);
end
...
for n=1:nSignals
    for m=1:nSignals
            ...
            plot(f{j},real(x_plot),'Color',ColorSet(j,:),'LineStyle','-','LineWidth',LineWidthSet(j));
            plot(f{j},imag(x_plot),'Color',ColorSet(j,:),'LineStyle','--','LineWidth',LineWidthSet(j));
            end    
...
%             axistight(gca,[0 0.05],'x','y');   
            ...
            if strcmpi(plotcoh,'yes') & n>m
                cla; hold on; grid on;
                for j=1:nSources
                x_plot=squeeze(x_coh{j}(n,m,:));
                plot(f{j},real(x_plot),'Color',ColorSet(j,:),'LineStyle','-','LineWidth',LineWidthSet(j));
                plot(f{j},imag(x_plot),'Color',ColorSet(j,:),'LineStyle','--','LineWidth',LineWidthSet(j));
                end    
                ylim([-1 1]*1.1);
...
%     
% % 
% % if max(diff(a(1:n)))./min(diff(a(1:n))) > 2
% % 	isXaxis=false;
% % end
% 
% 
